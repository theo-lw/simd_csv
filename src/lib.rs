/* Steps:
 *  1. Identify the quoted regions (done using the poly mult trick) 2. Identify the delimiters and the CRLF's
...
fn exists_missing_quotes(
    ...
    let prev_quoted_region_ended =
        !ended_quoted_region || quoted_regions & 1 == 1 || (delims_and_crlfs & 1 == 1);
    let starting_quoted_region_preceeded_by_delim =
        ended_delim || in_quoted_region == 1 || quoted_regions & 1 != 1 || ended_quoted_region;
...
pub fn decode_csv_structure(csv: &[u8], delim: u8) -> Result<Vec<Token>, ParseError> {
    ...
    unsafe {
        while csv_idx + STEP <= csv.len() {
            ...
            for _ in 0.._popcnt32(all_structural_chars) {
                ...
                let quotes_before = (_popcnt32(quotes & quote_mask) + num_carried_quotes) as usize;
    ...
    for (idx, elem) in csv[csv_idx..].iter().enumerate() {
        match (parse_state, *elem) {
            ...
            (ParseState::NonQuotedField, ch) if ch != delim && ch != LINE_FEED && ch != QUOTE => {}
...
mod tests {
    ...
    fn find_extract_csv_works() {
        ...
        match extracted.as_mut_slice() {
            [ref mut first, ref mut second, ref mut third, ref mut fourth, ref mut fifth, ref mut sixth, ref mut seventh] =>
            {
                assert_eq!(first.decode().unwrap(), b"a");
                assert_eq!(second.decode().unwrap(), b"b\"\r\n\",c");
                assert_eq!(third.decode().unwrap(), b" d ");
                assert_eq!(fourth.decode().unwrap(), b"e");
                assert_eq!(fifth.decode().unwrap(), b"");
                assert_eq!(sixth.decode(), None);
                assert_eq!(seventh.decode().unwrap(), b"asdasdasdasdasdasdasdasdasdas");
            }
    ...
    fn decode_csv_structure_works() {
        let mut csv = Vec::new();
        const LINE: &[u8] = b"hello, world ,\"404, 303\"\"\r\n\", ,\r\n";
        const NUM_LINES: usize = 1000;
        for _ in 0..NUM_LINES {
            csv.extend(LINE);
        }
        assert!(decode_csv_structure(&csv, b',').is_ok());
        assert!(extract_csv(&mut csv, b',').is_ok());
        assert_eq!(
        ...
        assert_eq!(
            decode_csv_structure(
                b"0123456789ABCDEF0123456789A,\"DE\r\n\
                  ...
                  0123456789ABCDEF0123456789A,\"DE\r\n\
                  012\",56789ABCDEF0123456789ABCDEF",
                b',',
            )
            .unwrap(),
